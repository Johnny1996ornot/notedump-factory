...
with col1:
    ...
    if not up:
        ...
        # 2. THE CSS TO FORCE THE INVISIBLE UPLOADER TO COVER EVERY SINGLE PIXEL
        st.markdown("""
        <style>
            ...
            /* Make the Streamlit UI completely transparent */
            ...
            /* THE ULTIMATE CLICK CATCHER: Force the actual HTML input tag to be massive */
            ...
            [data-testid="stFileUploadDropzone"] button {
                ...
                width: 100% !important; height: 100% !important;
                cursor: pointer !important;
                z-index: 9999 !important; /* GUARANTEES IT CATCHES CLICKS */
            }
        </style>
        """, unsafe_allow_html=True)
    ...
    # STATE 2: FILE HAS BEEN UPLOADED
        ...
        if st.session_state.get("current_file_key") != file_key:
            ...
            # --- 🚀 ADDED: VISUAL PROGRESS BAR & SPINNER ---
            progress_bar = st.progress(0)
            status_text = st.empty()

            status_text.markdown("⏳ **Uploading file securely...**")
            for percent_complete in range(0, 101, 20):
                time.sleep(0.05) # Quick visual fill to acknowledge upload
                progress_bar.progress(percent_complete)
            
            status_text.markdown(f"⚙️ **Processing `{up.name}`...**")
            # -----------------------------------------------
            ...
            # Clean up the UI after processing
            progress_bar.empty()
            status_text.empty()
