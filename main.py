...
/* Hide native spinning circle completely only for the final uploaded state */
[data-testid="stColumn"]:nth-child(1) [data-testid="stUploadedFile"] > div:first-child > div:first-child > svg { display: none !important; }
...
with col1:
    ...
    if not up:
        ...
        # 2. THE CSS TO FORCE THE INVISIBLE UPLOADER TO COVER IT PERFECTLY AND SHOW PROGRESS
        st.markdown("""
        <style>
            /* KILL THE PADDING DEAD ZONES so the button reaches the VERY TOP edge */
            ...
            /* Target the main uploader container and stretch it */
            ...
            /* HIDE ONLY THE DROPZONE (so the default button goes away), but keep it stretched */
            ...
            /* FORCE THE ACTUAL HTML INPUT TAG TO CATCH CLICKS EVERYWHERE */
            ...
            /* SHOW STREAMLIT'S NATIVE PROGRESS BAR WHILE UPLOADING */
            /* Because we only hid the dropzone, Streamlit's uploading card will pop up here naturally */
            [data-testid="stFileUploader"] > div[data-testid="stUploadedFile"] {
                opacity: 1 !important;
                position: absolute !important;
                ...
                width: 90% !important;
                z-index: 9999 !important; /* Forces it over your custom UI so you can see progress */
            }
        </style>
        """, unsafe_allow_html=True)
    ...
    # STATE 2: FILE HAS BEEN UPLOADED
        ...
        <style>
            ...
            /* Kill the dropzone completely once the file is ready */
            [data-testid="stFileUploadDropzone"] { 
        ...
        if st.session_state.get("current_file_key") != file_key:
            ...
            # Add a secondary spinner for Python processing time
            with st.spinner(f"Converting {up.name} to interactive notebook..."):
                try:
                    nav, slides = "", ""
