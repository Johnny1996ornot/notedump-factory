...
// FIX: Spawns items in the exact center of your screen *mapped onto the active page*
...
function applyCanvasSettings() {
    ...
    if (currentViewMode === 'single') { $('#canvas').css('height', pxH + 'px'); }
...
$(document).ready(function() {
    ...
    // FIX: Scroll Tracker sets active page automatically based on viewport center
    let scrollTimeout;
    $('#canvas-viewport').on('scroll', function() {
        if (currentViewMode !== 'scroll') return;
        if (scrollTimeout) return;
        scrollTimeout = setTimeout(() => {
            let viewportTop = $(this).scrollTop();
            let viewportMiddle = viewportTop + ($(this).innerHeight() / 2);
            let closestPageId = current;

            $('.page').each(function() {
                let pageTop = $(this)[0].offsetTop * currentZoom;
                let pageBottom = pageTop + ($(this).outerHeight() * currentZoom);
                if (viewportMiddle >= pageTop && viewportMiddle <= pageBottom) {
                    closestPageId = $(this).attr('id').replace('p-', '');
                }
            });

            if (closestPageId !== current) {
                current = closestPageId;
                $('.nav-link').removeClass('active-nav');
                $('#link-' + current).addClass('active-nav');
                
                let totalNav = $('.nav-link').not('.nav-chapter').length;
                let currentIndex = $('.nav-link').not('.nav-chapter').index($('#link-' + current)) + 1;
                $('#nb-page-indicator').text(`${currentIndex} / ${totalNav}`);
                
                let $currPage = $(`#p-${current}`);
                let h = parseInt($currPage.attr('data-page-height')) || 1054;
                $('#canvas-h-cm').val(Math.round((h / 37.795) * 10) / 10);
            }
            scrollTimeout = null;
        }, 50); 
    ...
    // FIX: Clicking a page manually updates active state immediately
    $(document).on('mousedown', '.page', function(e) {
        if (currentViewMode === 'scroll') {
            let clickedPageId = $(this).attr('id').replace('p-', '');
            if (current !== clickedPageId) {
                current = clickedPageId;
                $('.nav-link').removeClass('active-nav');
                $('#link-' + current).addClass('active-nav');
                let totalNav = $('.nav-link').not('.nav-chapter').length;
                let currentIndex = $('.nav-link').not('.nav-chapter').index($('#link-' + current)) + 1;
                $('#nb-page-indicator').text(`${currentIndex} / ${totalNav}`);
                let h = parseInt($(this).attr('data-page-height')) || 1054;
                $('#canvas-h-cm').val(Math.round((h / 37.795) * 10) / 10);
            }
        }
    });
